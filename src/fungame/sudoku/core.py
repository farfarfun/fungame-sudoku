...
logger = logging.getLogger("fungame")
...
class Sudoku(object):
            ...
            if isinstance(item, list):
                if item.count(val):
                    ...
                    if len(item) == 1:
                        ...
                        logger.debug("only one in row: answer={} at {}".format(self.value[r, i], (r, i)))
            ...
            if isinstance(item, list):
                if item.count(val):
                    ...
                    if len(item) == 1:
                        ...
                        logger.debug("only one in col: answer={} at {}".format(self.value[i, c], (i, c)))
                    ...
                    if isinstance(item, list):
                        for value in item:
                            if sum(map(lambda x: x.count(value), values)) == 1:
                                ...
                                logger.debug(
                                    "list val is only one in block: answer={} at {}".format(
    ...
    def _check_same_num(self):
        for b_r, b_c in self.base_points:
            ...
            for i in range(1, 10):
                result = map(
                    lambda x: 0 if not isinstance(x[1], list) else x[0] + 1 if x[1].count(i) else 0, enumerate(_data)
                )
                                ...
                                if isinstance(item, list):
                                    if item.count(i):
                                        ...
                                        if len(item) == 1:
                                            ...
                                            logger.debug(
                                                "block compare row: answer={} at {}".format(
                                                    self.value[row, col], (row, col)
                                                )
                                            )
                                ...
                                if isinstance(item, list):
                                    if item.count(i):
                                        ...
                                        if len(item) == 1:
                                            self.new_points.put((row, col))
                                            logger.debug(
                                                "block compare col: answer={} at {}".format(
                                                    self.value[row, col], (row, col)
                                                )
                                            )
                                            self.value[row, col] = item[0]
    ...
    def check_value(self):
        ...
        for b_r, b_c in self.base_points:
            ...
            for item in block:
                (lists if isinstance(item, list) else nums).append(item)
            if len(set(nums)) != len(nums):
                logger.debug("verify failed. dup in block {}".format((b_r, b_c)))
                return False  # 数字要不重复
            if len(list(filter(lambda x: len(x) == 0, lists))):
                return False  # 候选列表不能为空集
        return True
    ...
    def record_guess(self, point, index=0):
        ...
        logger.debug("added to LIFO queue: {}".format([x.point for x in self.recorder.queue]))
        ...
        logger.debug("guessing: answer={}/{} @{}".format(self.value[point], item, point))
        self.sudo_exclude()
    ...
    def recall(self):
        ...
        logger.debug("Recall! Try next possible in same point, {} @{}".format(item[index], point))
    ...
    def sudo_solve(self):
        ...
        while True:
            if self.check_value():
                ...
                logger.debug("current no. of fixed answers: {}".format(fixed_answer))
                if fixed_answer == 81:
                    break
                else:
                    # 获取最佳猜测点
                    point = self.get_best_point()
                    logger.debug("Adding new guessing in LIFO, {}".format(point))

                    # 记录并处理
                    self.record_guess(point)
                    logger.debug("guessed, current result:\n{}".format(self.value))
            else:
                # 出错，则回溯，尝试下一个猜测
                self.recall()
